...
export function KeyPositionsList({ positions }: KeyPositionsProps) {
  const stanceColor = (s: string) => {
    if (s.includes('support') || s === 'pro-regulation' || s === 'increase' || s === 'top priority') return 'bg-green-500/20 text-green-700 dark:text-green-400';
    if (s.includes('oppos') || s === 'reduce' || s === 'sceptical') return 'bg-red-500/20 text-red-700 dark:text-red-400';
