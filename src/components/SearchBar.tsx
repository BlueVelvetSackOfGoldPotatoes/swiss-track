...
const SearchBar = () => {
  ...
  const handleSearch = (q: string) => {
    ...
    const countrySet = new Map<string, { code: string; name: string }>();
    actors.forEach(a => {
      if (!countrySet.has(a.countryId)) {
        countrySet.set(a.countryId, { code: a.countryId.toUpperCase(), name: a.canton || a.countryId });
      }
    });
    ...
    const matched: SearchResult[] = [
      ...
      ...Array.from(countrySet.values())
        .filter(c => c.name.toLowerCase().includes(lower) || c.code.toLowerCase().includes(lower))
        .map(c => ({ type: 'country' as const, id: c.code.toLowerCase(), label: c.name, detail: c.code })),
    ];
